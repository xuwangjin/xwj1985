...
func (this *Applier) InitiateHeartbeat() {
	...
	injectHeartbeat := func() error {
		...
		//更新changelog表中hint=heartbeat的记录做心跳数据
...
func (this *Applier) CalculateNextIterationRangeEndValues() (hasFurtherRange bool, err error) {
	...
	for i := 0; i < 2; i++ {
		...
		//此处迭代完毕Next()方法为false hasFurtherRange=false
		for rows.Next() {
...
func (this *Applier) buildDMLEventQuery(dmlEvent *binlog.BinlogDMLEvent) (results []*dmlBuildResult) {
	...
	case binlog.InsertDML:
		{
			/*
				insert 被处理成replace
				apply binlog events 跟copy数据两个过程不是绝对串行的
				可能存在数据冲突的可能故要改写成replace
			*/
	...
	case binlog.UpdateDML:
		{
			/*
				如果update事件中唯一索引列被修改则处理顺序是先delete在进行insert
			*/
...
// 解析binlog-dmlEvents并应用到迁移中间表
func (this *Applier) ApplyDMLEventQueries(dmlEvents []*binlog.BinlogDMLEvent) error {

	var totalDelta int64

	err := func() error {
		//开启事物this.migrationContext.DMLBatchSize个事物一次性提交
		tx, err := this.db.Begin()
		if err != nil {
			return err
		}
