...
type EventsStreamer struct {
	...
	listeners                []*BinlogEventListener //listen & handle
...
func (this *EventsStreamer) notifyListeners(binlogEvent *binlog.BinlogDMLEvent) {
	...
	for _, listener := range this.listeners {
		...
		if listener.async {
			...
			//此处只能采用串行模式否则有可能在消费binlogEvents时发生并行导致数据不一致
			listener.onDmlEvent(binlogEvent)
		}
	}
}
...
func (this *EventsStreamer) StreamEvents(canStopStreaming func() bool) error {
	...
	for {
		...
		//解析binlog流 并传递到chan eventsChannel供下游消费this.notifyListeners(binlogEntry.DmlEvent)
		//最终符合条件的binlog流将被注册的监听器所消费处理[gho & ghc两个表的监听器]
